package com.distributed_task_framework.perf_test_app.configuration;
...
@Configuration
@OpenAPIDefinition(
    info = @Info(title = "Perf test application API", version = "v1")
)
        ...
        return new OpenApiWebMvcResource(
            ...
            protected String getServerUrl(HttpServletRequest request, String apiDocsUrl) {
                ...
                if ("https".equalsIgnoreCase(request.getHeader("x-forwarded-proto"))) {
                    return serverUrl.replaceFirst("http", "https");
                }
                ...
                if (StringUtils.hasText(request.getHeader("forwarded"))) {
                    Map<String, String> forwarded = Arrays.stream(
                        ...
                        .map(keyVal -> Map.entry(keyVal.split("=")[0], keyVal.split("=")[1]))
